...
SELECT nome_cliente, YEAR(data_nascimento) as ano_nascimento,
...
SELECT categoria, COUNT(id_produto) as contagem
FROM tb_produto
GROUP BY categoria;
...
-- 6
SELECT nome_cliente FROM vw_cliente_detalhes
WHERE cidade = "São Paulo";

-- 7
SELECT nome_produto, categoria FROM vw_produto_estoque
WHERE estoque > 150;

-- 8
SELECT nome_cliente FROM vw_clientes_nascimento
WHERE mes_nascimento = 2;

-- 9
SELECT nome_produto FROM vw_produtos_precos
WHERE lucro > 5.00;

-- 10
SELECT categoria FROM vw_categorias
WHERE contagem > 10;
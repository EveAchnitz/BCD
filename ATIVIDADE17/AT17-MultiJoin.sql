...
FROM tb_pedido_roupa JOIN tb_roupa 
ON tb_roupa.id_roupa = tb_pedido_roupa.ROUPA_id_roupa
JOIN tb_cor ON tb_cor.id_cor = tb_roupa.COR_id_cor
JOIN tb_cliente ON tb_cliente.id_cliente = tb_pedido_roupa.CLIENTE_id_cliente
WHERE tb_cliente.COR_id_cor_favorita  = tb_roupa.COR_id_cor
ORDER BY tb_cliente.primeiro_nome;

-- Exercício 2:
SELECT cl.primeiro_nome AS nome, cl.sobrenome, c.nome AS Cor_Favorita
FROM tb_cliente cl JOIN tb_cor c ON c.id_cor = cl.COR_id_cor_favorita
LEFT JOIN tb_pedido_roupa pd ON pd.CLIENTE_id_cliente = cl.id_cliente
...
SELECT cl.primeiro_nome, cl.sobrenome AS nome_completo, r.nome AS nome_roupa, c.nome AS cor_roupa, pd.quantidade, pd.data_pedido
FROM tb_pedido_roupa pd JOIN tb_cliente cl 
ON pd.CLIENTE_id_cliente = cl.id_cliente
JOIN tb_roupa r ON pd.ROUPA_id_roupa = r.id_roupa
JOIN tb_cor c ON c.id_cor = r.COR_id_cor;

select * from vw_detalhes_pedidos;
-- prof, não consegui colocar o nome e o sobrenome do cliente na mesma coluna :(

-- Exercício 5: Usando a view vw_detalhes_pedidos, liste todos os pedidos de roupas na cor "Vermelho".
SELECT nome_roupa FROM vw_detalhes_pedidos
